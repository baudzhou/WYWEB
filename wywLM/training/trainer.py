...
from ..data import (
    ...
    SequentialSampler,
    AsyncDataLoader,
    collate_with_batch_correct,
)
from ..utils import get_logger, MeterBuffer, gpu_mem_usage
...
class TrainerState:
    ...
    def update_step(self, loss, examples, loss_scale, output=None):
...
class DistributedTrainer:
        ...
        **kwargs,
        ...
        if training_steps is None:
            training_steps = getattr(
                args,
                "training_steps",
                (len(train_data) + self.args.train_batch_size - 1)
        ...
        self.meter = MeterBuffer(window_size=self.report_interval)
        self.model = self._setup_model(args, model)
        self.post_loss_fn = None
        # Tensorboard logger
        if args.rank == 0:
            self.tb_name = os.path.join(self.output_dir, "tblogger")
            if not os.path.exists(self.tb_name):
                os.makedirs(self.tb_name)
            self.tblogger = SummaryWriter(self.tb_name)
        ...
        if len(self.args.resume_opt_path) > 0:
            self.optimizer = torch.load(self.args.resume_opt_path)
        else:
    ...
    def train(self):
        ...
        for n_epoch in range(self.trainer_state.epochs, self.training_epochs):
            ...
            train_dataloader = DataLoader(
                ...
                pin_memory=True,
                collate_fn=collate_with_batch_correct,
            )
            torch.cuda.empty_cache()
            for step, batch in enumerate(AsyncDataLoader(train_dataloader, 1000)):
                if self.trainer_state.steps >= self.training_steps:
                    break
                # if step < 1510 and step >= 1500 and rank == 1:
                #   self.report_interval = 1
                #   torch.save(batch, str(step))
                bs_scale = 1
    ...
    def save_model(self, args, checkpoint_dir, chk_postfix, model, optimizer):
        ...
        opt_path = os.path.join(checkpoint_dir, f"opt.bin")
    ...
    def _train_step(self, data, bs_scale):
        ...
        while not go_next:
            ...
            for i, sub in enumerate(data_chunks):
                ...
                if isinstance(output, dict):
                    ...
                    output = {"loss": loss, "batch_size": sub_size}
        ...
        self.meter.update(
            ...
            * self.optimizer.optimizer.get_group_lr_sch(
                self.optimizer.param_groups[0], self.trainer_state.steps
            ),
            **output,
        )
        if self.trainer_state.steps % self.report_interval == 0:
            self.trainer_state.report_state()
            self.report_meter()
            if self.args.rank == 0:
                for k, v in output.items():
                    self.tblogger.add_scalar(k, v, self.trainer_state.steps)
