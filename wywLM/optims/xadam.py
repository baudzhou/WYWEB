...
def adamw(
    ...
    if layer_adapt:
        # dx = update - data
        x_norm = torch.norm(data)
        lr = np.clip(lr, 1e-10, float("inf"))
        g_norm = torch.norm(update / lr)
        ratio = torch.where(
            x_norm > 0.0, torch.where(g_norm > 0.0, x_norm / g_norm, 1.0), 1.0
        )
        update.add_(ratio, update)
